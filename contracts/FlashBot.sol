...
contract FlashBot is Ownable {
    ...
    // ACCESS CONTROL
    // Only the `permissionedPairAddress` may call the `AntfarmCall` function
    ...
    /// The callback function on different DEX are not same, so use a fallback to redirect to AntfarmCall
    fallback(bytes calldata _input) external returns (bytes memory) {
        ...
        AntfarmCall(sender, amount0, amount1, data);
    }
        ...
        )
    {
        ...
        (address pool0Token0, address pool0Token1) = (IAntfarmPair(pool0).token0(), IAntfarmPair(pool0).token1());
        (address pool1Token0, address pool1Token1) = (IAntfarmPair(pool1).token0(), IAntfarmPair(pool1).token1());
    ...
    {
        (uint256 pool0Reserve0, uint256 pool0Reserve1, ) = IAntfarmPair(pool0).getReserves();
        (uint256 pool1Reserve0, uint256 pool1Reserve1, ) = IAntfarmPair(pool1).getReserves();
    ...
    function flashArbitrage(address pool0, address pool1) external {
        ...
        // avoid stack too deep error
        {
            ...
            IAntfarmPair(info.lowerPool).swap(amount0Out, amount1Out, address(this));
        }
    ...
    function AntfarmCall(
        ...
        IAntfarmPair(info.targetPool).swap(amount0Out, amount1Out, address(this));
    ...
    function getProfit(address pool0, address pool1) external view returns (uint256 profit, address baseToken) {
        ...
        baseToken = baseTokenSmaller ? IAntfarmPair(pool0).token0() : IAntfarmPair(pool0).token1();
